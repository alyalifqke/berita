                ...
                <div class="row g-4">
                    <div class="col-lg-8">
                        ...
                        <div class="position-relative rounded overflow-hidden mb-3">
                            <img src="{{ asset('storage/' . $news->image) }}" class="img-zoomin img-fluid rounded w-100" alt="">
                            <div class="position-absolute text-white px-4 py-2 bg-primary rounded" style="top: 20px; right: 20px;">                                              
                                {{ $news->category->name }}
                            </div>
                        </div>
                        <div class="d-flex justify-content-between">
                            <a href="#" class="text-dark link-hover me-3"><i class="fa fa-clock"></i> {{ \Carbon\Carbon::parse($news->updated_at)->translatedFormat('d F Y') }}</a>
                            <a href="#" class="text-dark link-hover me-3"><i class="fa fa-eye"></i> {{ $news->views }} Kali</a>
                            <a href="#" class="text-dark link-hover me-3"><i class="fa fa-comment-dots"></i> {{ $news->comments->count() }} Komentar</a>
                        ...
                        {{-- <p class="my-4">yang mana iniLorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy Lorem Ipsum has been the industry's standard dummy type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic
                            Lorem Ipsum is simply dummy
                        </p>
                        </p> --}}
                                             
                        <div class=" d-flex">
                            @for ($i = 0; $i < $latestNews->count(); $i += 1)
                            <div class="me-2">                               
                                @foreach ($latestNews->slice($i, 1) as $ln)                               
                                <a href="{{ $ln->slug }}"><span class="fas fa-check-circle mb-3 fs-5"> cek {{ Str::limit($ln->title, 30, '...') }}</span></a>
                                @endforeach                             
                            </div>
                            @endfor
                            @for ($i = 0; $i < $latestNews->count(); $i += 1)
                            <div class="ms-2">
                                @foreach ($latestNews->slice($i, 1) as $ln)
                                <span class="fas fa-check-circle mb-3 fs-5"> {{ Str::limit($ln->title, 40, '...') }}
                                @endforeach                                
                            </div>
                            @endfor 
                        </div>
                        ...
                        <div class="row g-4">
                            <div class="col-12">
                                <div class="p-3 rounded border">
                                    ...
                                    @for ($i = 0; $i < $latestNews->count(); $i += 1)
                                    <div class="row g-4">
                                        @foreach ($latestNews->slice($i, 1) as $laten)
                                        <div class="col-12">
                                            <div class="row g-4 align-items-center features-item">
                                                <div class="col-4">
                                                    <div class="rounded-circle position-relative">
                                                        <div class="overflow-hidden rounded-circle">
                                                            <img src="{{ asset('storage/' . $laten->image) }}" class="img-zoomin img-fluid rounded-circle w-100" alt="">
                                                        </div>
                                                        <span class="rounded-circle border border-2 border-white bg-primary btn-sm-square text-white position-absolute" style="top: 10%; right: -10px;">{{ $laten->views }}</span>
                                                    </div>
                                                </div>
                                                <div class="col-8">
                                                    <div class="features-content d-flex flex-column">
                                                        <p class="text-uppercase mb-2">{{ $laten->category->name }}</p>
                                                        <a href="{{ $laten->slug }}" class="h6">
                                                           {{ Str::limit($laten->title, 20, '...') }}
                                                        </a>
                                                        <small class="text-body d-block"><i class="fas fa-calendar-alt me-1"></i> {{ \Carbon\Carbon::parse($laten->updated_at)->translatedFormat('d F Y') }}</small>
                                                    </div>
                                                </div>
                                            </div>
                                            @endforeach
